#!/usr/bin/env python
"""Plot Visual_Z effects on RT_onset, RT_critical, and ACC."""
...
def fit_lmer_visual_rt_with_r(input_csv, dv):
    r_code = r'''
...
x_grid <- seq(min(dat_rt$Visual_Z), max(dat_rt$Visual_Z), length.out = 240)
...
slope_vec <- function(condition_name) {
  ...
  if (condition_name == "sensorimotor") {
    vec["conditionsensorimotor:Visual_Z"] <- 1
  } else if (condition_name == "Voe") {
    vec["conditionVoe:Visual_Z"] <- 1
  }
  vec
}
...
'''
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        script_path = temp_path / "fit_visual_rt_lmer.R"
        lines_path = temp_path / "visual_rt_lines.csv"
        summary_path = temp_path / "visual_rt_summary.csv"
...
def fit_lmm_visual_rt(input_csv, dv):
    try:
        return fit_lmer_visual_rt_with_r(input_csv, dv)
    ...
    def design_row(group, visual_z):
        ...
        if group == "Action":
            ...
            row["C(condition)[T.sensorimotor]:Visual_Z"] = visual_z
        elif group == "Voe":
    ...
    fixed_lines = {}
    slope_vectors = {
        "Semantic": {"Visual_Z": 1.0},
        "Action": {"Visual_Z": 1.0, "C(condition)[T.sensorimotor]:Visual_Z": 1.0},
...
def fit_glmer_visual_acc_with_r(input_csv):
    r_code = r'''
suppressPackageStartupMessages(library(lme4))
...
x_grid <- seq(min(dat_acc$Visual_Z), max(dat_acc$Visual_Z), length.out = 240)
conditions <- c("categorization", "Voe", "sensorimotor")
groups <- c("Semantic", "Voe", "Action")
...
'''
    with tempfile.TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)
        script_path = temp_path / "fit_visual_acc_glmer.R"
        lines_path = temp_path / "visual_acc_lines.csv"
        summary_path = temp_path / "visual_acc_summary.csv"
        script_path.write_text(r_code, encoding="utf-8")
...
def fit_logit_visual_acc(input_csv):
    try:
        return fit_glmer_visual_acc_with_r(input_csv)
    except Exception as exc:
        warnings.warn(f"Falling back to statsmodels BinomialBayesMixedGLM for ACC: {exc}")

    data = pd.read_csv(input_csv)
    data["condition"] = pd.Categorical(
        data["condition"], categories=["sensorimotor", "categorization", "Voe"]
    )
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
...
def plot_rt_panel(ax, empirical, fixed_lines, slope_summaries, dv):
    ...
    ax.set_title(f"{VISUAL_LABEL} effect on {LOG_DV_LABELS[dv]}", fontsize=16, pad=12)
    ax.set_xlabel(VISUAL_LABEL, fontsize=14)
...
def plot_acc_panel(ax, empirical, fixed_lines, slope_summaries):
    ...
    note_y = {"Action": 0.16, "Voe": 0.10, "Semantic": 0.04}
    for group in ["Action", "Voe", "Semantic"]:
        slope = slope_summaries[group]
        ax.text(
            0.98,
    ...
    ax.set_title(f"{VISUAL_LABEL} effect on logit(ACC)", fontsize=16, pad=12)
    ax.set_xlabel(VISUAL_LABEL, fontsize=14)
    ax.set_ylabel("logit(ACC)", fontsize=14)
    model_vals = np.concatenate([fixed_lines[group]["mean"] for group in PLOT_ORDER])
    pad = (float(model_vals.max()) - float(model_vals.min())) * 0.22
    ax.set_ylim(float(model_vals.min()) - pad, float(model_vals.max()) + pad)
    ax.set_xlim(
        float(fixed_lines["Semantic"]["x"].min()) - 0.12,
        float(fixed_lines["Semantic"]["x"].max()) + 0.22,
    )
...
def save_summary(output_dir, onset_summaries, critical_summaries, acc_summaries):
    path = output_dir / "tables" / "visual_z_rt_effects_summary.csv"
    with open(path, "w", newline="", encoding="utf-8") as handle:
        writer = csv.writer(handle)
        writer.writerow(["section", "group", "value_1", "value_2", "value_3", "extra"])
        for group in PLOT_ORDER:
...
def main():
    ...
    onset_lines, onset_summaries = fit_lmm_visual_rt(input_csv, "RT_onset")

    empirical_critical = load_empirical_rt(input_csv, "RT_critical")
    critical_lines, critical_summaries = fit_lmm_visual_rt(input_csv, "RT_critical")

    empirical_acc = load_empirical_acc(input_csv)
    acc_lines, acc_summaries = fit_logit_visual_acc(input_csv)
    ...
    fig.suptitle(
        "Visual complexity predicts both encoding-related and post-event response times, alongside observed accuracy",
        fontsize=17,
        y=0.98,
    )
    ...
    png_path = fig_dir / "visual_z_rt_effects.png"
    pdf_path = fig_dir / "visual_z_rt_effects.pdf"
