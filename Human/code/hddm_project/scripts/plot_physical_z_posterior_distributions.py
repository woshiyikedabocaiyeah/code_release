#!/usr/bin/env python
"""Plot posterior distributions of Physical_z coefficients across conditions."""
...
def load_physical_slopes(output_dir, chains):
    models = load_models(output_dir, "regression", chains)
    return summarize_physical_slopes(models)


def summarize_physical_slopes(models):
    node_names = {
...
def save_summary(output_dir, slopes):
    path = output_dir / "tables" / "physical_z_posterior_distribution_summary.csv"
...
def plot_posterior_distributions(ax, slopes):
    ...
    ax.set_title(
        r"Model results: posterior distributions of $\mathrm{Physical}_{z}$ coefficients",
        fontsize=12,
        pad=10,
    )
...
def main():
    ...
    slopes = load_physical_slopes(output_dir, args.chains)
    ...
    png_path = fig_dir / "physical_z_posterior_distributions.png"
    pdf_path = fig_dir / "physical_z_posterior_distributions.pdf"
