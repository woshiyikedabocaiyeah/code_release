...
# -*- coding: utf-8 -*-
"""
RynnBrain-8B inference script for experiment pipeline.
...
class RynnBrain8BInference:
    """RynnBrain-8B inference with three-layer embedding extraction."""
...
def main():
    parser = argparse.ArgumentParser(description="RynnBrain-8B inference for experiment")
    parser.add_argument(
        "--model",
        "-m",
        type=str,
        default=str(PROJECT_ROOT / "model_weights" / "RynnBrain-8B" / "RynnBrain-8B"),
    ...
    parser.add_argument("--video-id", type=str, required=True, help="Video ID for output names")
    parser.add_argument(
        "--output",
        "-o",
        type=str,
