...
# -*- coding: utf-8 -*-
"""
RoboBrain2.5-8B-NV inference script for experiment pipeline.
...
class RoboBrain25Inference:
    """RoboBrain2.5-8B inference with three-layer embedding extraction."""
...
def main():
    parser = argparse.ArgumentParser(description="RoboBrain2.5-8B Inference for Experiment")
    parser.add_argument(
        "--model",
        "-m",
        type=str,
        default=str(PROJECT_ROOT / "model_weights" / "RoboBrain2.5" / "RoboBrain2.5-8B-NV"),
    ...
    try:
        runner = RoboBrain25Inference(args.model, args.output)
    except Exception as exc:
        print(
            json.dumps(
                {
