...
def build_stats(results: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    stats: dict[str, dict[str, Any]] = {}
    for row in results:
        key = f"{row.get('model')}_{row.get('mode')}_{row.get('prompt_type', 'simple')}"
...
def combine_setting(source_root: Path, output_root: Path, setting: str, filename: str) -> dict[str, Any]:
    ...
    payload = {
        "experiment_info": {
            ...
            "model_statistics": build_stats(results),
        },
        "results": results,
    }
    with out_file.open("w", encoding="utf-8") as file:
        json.dump(payload, file, indent=2, ensure_ascii=False)
    return {"setting": setting, "output": str(out_file), "source_files": len(source_files), "total_results": len(results)}


def main() -> None:
    parser = argparse.ArgumentParser(description="Combine per-model semantic Yes/No result JSON files.")
    ...
    print(f"Combined semantic Yes/No outputs written under {output_root}")
